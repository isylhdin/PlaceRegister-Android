...
public class TypesUtil {
	 ...
	 * 
	 * @param types
	 *            Different types belonging to a place
	 * @param existingTypes
	 *            types supported by the application
	...
	/**
	 * For the moment, if types.size() > 1, we only take into account the first
	 * item of the list.
	 * 
	 * @param types
	 * @return the id of the corresponding marker (in the res folder) to display
	 *         on map
	 */
	public static int getVisitedMarkerTypeId(List<String> types) {

		int id = 0;

		if (types.size() != 0) {
			PlaceType placeType = PlaceType.getPlaceByType(types.get(0));
			id = placeType.getMarkerId();
	 ...
	 * 
	 * @param different
	 *            types returned by google map web services. A place can have
	 *            several types
